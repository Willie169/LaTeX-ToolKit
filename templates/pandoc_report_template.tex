...
\usepackage{xeCJK,titlesec,titling,setspace,needspace}
\usepackage{amsthm,amsmath,amsfonts,amssymb,dsfont,stmaryrd,unicode-math,mathtools,mathrsfs}
\usepackage{diagbox,cellspace,array,enumitem,graphicx,caption,subcaption,float,placeins,xcolor}
...
\ProvideDocumentCommand{\bc}{}{\begin{center}}
...
\ProvideDocumentCommand{\ec}{}{\end{center}}
...
\ProvideDocumentCommand{\itg}{m m o o}{%
  \IfNoValueTF{#4}{%
    \IfNoValueTF{#3}{%
      \int #1\,\mathrm{d}{#2}%           % \itg{f}{x}
    }{%
      \int_0^{#1} #2\,\mathrm{d}{#3}%    % \itg{a}{f}{x}
    }%
  }{%
    \int_{#1}^{#2} #3\,\mathrm{d}{#4}%   % \itg{a}{b}{f}{x}
  }%
}
\let\La=\Leftarrow
\let\la=\leftarrow
...
\let\lra=\leftrightarrow
\ProvideDocumentCommand{\mt}{m}{\[#1\]}
\ProvideDocumentCommand{\md}{m}{\mathds{#1}}
\ProvideDocumentCommand{\me}{m}{\(#1\)}
